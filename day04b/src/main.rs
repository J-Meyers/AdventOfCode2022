    ...
    if let Ok(lines) = read_lines("./day4.txt") {
        for line in lines {
            if let Ok(line_s) = line {
                ...
                if overlaps_at_all(lhs, rhs) {
                    num_contained += 1;
                }
            }
        }
        println!("Pairs with any overlap {num_contained}");
    } else {
        eprintln!("Failed to Open File!");
    }
}

fn overlaps_at_all(lhs: (usize, usize), rhs: (usize, usize)) -> bool {
    if lhs.0 <= rhs.1 && lhs.1 >= rhs.0 {
        true
    } else if rhs.0 <= lhs.1 && rhs.1 >= lhs.0 {
        true
    } else {
        false
    }
